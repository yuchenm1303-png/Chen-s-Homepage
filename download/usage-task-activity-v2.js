...
function captureClientActivity(rail, expectedLength) {
  ...
  return Array.isArray(stored) && stored.length === expectedLength ? stored.slice() : Array(expectedLength).fill(false);
}
...
function formatBucketLabel(bucket, spec, detailed = false) {
  if (spec.mode === "day" || spec.key === "7d") {
    return new Intl.DateTimeFormat("zh-CN", {
      ...(detailed ? { year: "numeric" } : {}), month: "2-digit", day: "2-digit",
...
function renderLegend() {
  ...
  note.textContent = "紫色仅代表最近仍有更新的真实运行任务";
  nodes.push(note);
...
async function refreshLifecycle() {
  ...
  try {
    ...
    const { data, error } = await lifecycleClient.rpc("get_listing_task_lifecycle_activity_v1", {
      p_view: monitorView(), p_hours: 168, p_days: 30
    });
...
function install() {
  ...
  if (usersPanel) {
    new MutationObserver(() => window.setTimeout(renderLatest, 0)).observe(usersPanel, { childList: true });
  }
  const generatedAt = document.getElementById("generatedAt");
  if (generatedAt) {
    new MutationObserver(() => scheduleRefresh()).observe(generatedAt, { childList: true, characterData: true, subtree: true });
  }
