...
// First-version Milky Way geometry + Astra filtered points.
// Major stars are now deliberately resolved in screen pixels so their stellar body
// remains visible at full-page scale instead of collapsing back into tiny light dots.
...
void main() {
  ...
  // Slow independent flow; geometry stays anchored to the authored galaxy path.
  float flowSpeed = mix(0.34, 0.72, depth) * (0.72 + 0.28 * aRate);
  float flowAmount = mix(0.004, 0.018, depth);
  ...
  float flowMask = mix(0.18, 1.0, structural) * mix(1.0, 0.42, hero);
  vec2 flowDirection = normalize(vec2(0.70, 1.0));
  vec2 flowAcross = vec2(-flowDirection.y, flowDirection.x);
  p.xy += flowDirection
    * (sin(phaseX + uTime * flowSpeed) - sin(phaseX))
    * flowAmount * flowMask;
  p.xy += flowAcross
    * (cos(phaseY + uTime * flowSpeed * 0.73) - cos(phaseY))
    * flowAmount * 0.40 * flowMask;

  p.xy += uPointer * mix(0.0035, 0.020, depth);
  vec2 pointerWorld = vec2(uPointer.x * uAspect, uPointer.y);
  vec2 delta = p.xy - pointerWorld;
  float repel = exp(-dot(delta, delta) * 15.0)
    * uPointerActive * (0.002 + 0.012 * depth);
  p.xy += normalize(delta + vec2(0.0001)) * repel;
  p.xy = rotate2d(-0.017 + uPointer.x * 0.006) * p.xy;

  // Same full-screen expansion as the approved first-version composition.
  p.x = (p.x - 0.535) * 1.72;
  p.y *= 1.035;
  ...
  float twinkle = 0.95 + 0.05 * basePulse;
  twinkle = mix(twinkle, 0.90 + 0.09 * basePulse + 0.02 * secondPulse, stream);
  twinkle = mix(twinkle, 0.82 + 0.15 * basePulse + 0.04 * secondPulse, bright);
  twinkle = mix(twinkle, 0.76 + 0.19 * basePulse + 0.055 * secondPulse, hero);

  float particleDiameter = max(0.15, aSize * uDpr)
    * mix(0.82, 1.22, depth);

  // Stronger separation between the quiet field and resolved main stars.
  float starBrightness = (0.45 + aSize * 0.18) * background;
  starBrightness += (0.86 + aSize * 0.33) * stream;
  starBrightness += (3.35 + aSize * 0.44) * bright;
  starBrightness += (7.10 + min(aSize, 11.0) * 0.10) * hero;
  ...
  float saturation = 1.00 + stream * 0.08 + bright * 0.68 + hero * 0.88;
  vColor = clamp(mix(vec3(luma), aColor, saturation), 0.0, 1.45);
  vOpacity = min(1.0,
    aAlpha * reveal * (0.92 + twinkle * 0.08) * (1.0 + bright * 0.18 + hero * 0.10));
  vBrightness = starBrightness * twinkle;
  vRayStrength = smoothstep(2.8, 6.2, starBrightness);
  vParticleDiameter = particleDiameter;
  vPulse = twinkle;
  vKind = aKind;

  // This is the key change: resolved stars get enough screen-space area for the
  // white core, colour shell and diffraction geometry to be perceptible.
  float brightSprite = (14.0 + aSize * 1.30) * uDpr;
  float heroSprite = (24.0 + aSize * 1.70) * uDpr;
  gl_PointSize = max(particleDiameter,
    4.0 + bright * brightSprite + hero * heroSprite);
}
`;
...
void main() {
  ...
  // Quiet galaxy-body stars: Astra-style filtered points, no broad halo.
  if (major < 0.5) {
    ...
    float disc = pow(1.0 - smoothstep(0.04, 0.82, r), 3.0);
    float alpha = mix(astraFilteredCore(pixel, 0.150904), disc, resolved) * vOpacity;
    if (alpha <= 0.0) discard;
    float whiteCore = gauss(r*r, 36.0) * smoothstep(0.95, 2.45, vBrightness);
    vec3 emission = mix(vColor, vec3(1.0), whiteCore * 0.70) * vBrightness;
  ...
  // Resolved stellar body: visible white photosphere, coloured temperature shell,
  // then diffraction. This is intentionally much larger than the previous sub-pixel body.
  float needle = gauss(r2, 360.0);
  float whiteDisk = 1.0 - smoothstep(0.050, 0.135, r);
  float photosphere = gauss(r2, 82.0);
  float innerCorona = gauss(r2, 28.0);
  float temperatureShell = exp(-pow((r - 0.205) * 11.5, 2.0));
  float airyRing = exp(-pow((r - 0.335) * 24.0, 2.0));
  float microAura = gauss(r2, 8.5) * (1.0 - smoothstep(0.58, 0.96, r));

  // Thin photographic diffraction, strong only on hero stars.
  float horizontal = exp(-abs(q.y) * 46.0)
    * exp(-abs(q.x) * 1.85)
    * (1.0 - smoothstep(0.16, 1.0, abs(q.x)));
  float vertical = exp(-abs(q.x) * 46.0)
    * exp(-abs(q.y) * 1.85)
    * (1.0 - smoothstep(0.16, 1.0, abs(q.y)));
  float mainCross = max(horizontal, vertical);

  vec2 diagCoord = vec2(q.x + q.y, q.y - q.x) * 0.70710678;
  float diagonal = max(
    exp(-abs(diagCoord.y) * 58.0) * exp(-abs(diagCoord.x) * 2.8),
    exp(-abs(diagCoord.x) * 58.0) * exp(-abs(diagCoord.y) * 2.8)
  );

  float spikeGain = (0.070 * bright + 0.50 * hero) * vRayStrength;
  float diagonalGain = 0.095 * hero * vRayStrength;
  float spikes = mainCross * spikeGain + diagonal * diagonalGain;

  // White-hot centre; temperature colour is concentrated in a clear annulus.
  float tintBand = smoothstep(0.115, 0.19, r)
    * (1.0 - smoothstep(0.36, 0.56, r));
  vec3 shellColor = mix(vec3(1.0), vColor, tintBand * (0.98 + hero * 0.10));
  float shellLuma = dot(shellColor, vec3(0.2126, 0.7152, 0.0722));
  shellColor = mix(shellColor, vec3(shellLuma), smoothstep(0.58, 0.98, r) * 0.70);
  vec3 spikeColor = mix(vec3(1.0), vColor, 0.18);

  float bodyShape = max(whiteDisk, photosphere * 0.96)
    + innerCorona * 0.34
    + temperatureShell * 0.16
    + airyRing * 0.055
    + microAura * 0.028;

  vec3 bodyEnergy = shellColor * (
      needle * 2.30
    + whiteDisk * 1.35
    + photosphere * 0.92
    + innerCorona * 0.34
    + temperatureShell * 0.22
    + airyRing * 0.060
    + microAura * 0.022
  );

  vec3 emission = bodyEnergy + spikeColor * spikes;
  float alpha = vOpacity * clamp(bodyShape + spikes, 0.0, 1.0);
  if (alpha <= 0.0004) discard;

  // Selective bloom only receives the compact hot body and a touch of hero spike energy.
  if (uBloomOnly > 0.5) {
    float bloomCore = needle * 1.70
      + whiteDisk * 0.80
      + photosphere * 0.38
      + innerCorona * 0.08
      + mainCross * (0.02 * bright + 0.09 * hero);
    vec3 bloomColor = mix(vec3(1.0), vColor, 0.20);
    outColor = vec4(
...
vec3 sourceAt(vec2 uv) {
  ...
  if (uExtract > 0.5) {
    ...
    c *= smoothstep(uThreshold, uThreshold + 0.11, l);
  }
  return c;
}
void main() {
  vec2 stepUv = uTexel * uDirection;
  vec3 c = sourceAt(vUv) * 0.36;
  c += (sourceAt(vUv + stepUv * 0.90) + sourceAt(vUv - stepUv * 0.90)) * 0.25;
  c += (sourceAt(vUv + stepUv * 1.85) + sourceAt(vUv - stepUv * 1.85)) * 0.07;
  outColor = vec4(c, 1.0);
}
`;
...
void main() {
  ...
  vec3 bloom = texture(uBloom0, vUv).rgb * 0.70
             + texture(uBloom1, vUv).rgb * 0.23
             + texture(uBloom2, vUv).rgb * 0.07;
  vec3 mapped = acesFilmic(scene + bloom * uBloomIntensity);
...
function pickColor(scale = 1) {
  ...
  const noise = 0.965 + random() * 0.07;
  return entry.color.map(channel => Math.min(1.18, channel * noise * scale));
}

const PRIMARY_PATH = [
  [-0.46, -1.30], [-0.16, -0.98], [0.16, -0.70], [0.43, -0.43],
  [0.67, -0.12], [0.82, 0.19], [0.98, 0.47], [1.22, 0.78], [1.53, 1.19],
];
const SECONDARY_PATH = [
  [-0.08, -1.30], [0.18, -1.01], [0.38, -0.69], [0.58, -0.40],
  [0.83, -0.13], [1.04, 0.19], [1.20, 0.54], [1.39, 0.90], [1.69, 1.24],
];
...
function addBackground(count) {
  for (let i = 0; i < count; i++) {
    ...
    const alpha = rare > 0.985 ? 0.24 + random()*0.22 : 0.040 + random()*0.14;
    pushStar(-2.22 + random()*4.44, -1.20 + random()*2.40, -0.94 + random()*1.88,
      pickColor(0.84), size, alpha, random()*39, 0.45 + random()*0.90, 0);
  }
}
function streamWidth(t, secondary = false) {
  const pulse = 0.5 + 0.5 * Math.sin(t*Math.PI*4.6 + 0.9);
  return (secondary ? 0.082 : 0.125) + pulse * (secondary ? 0.080 : 0.155);
}
function dustLaneOffset(t) { return -0.026 + Math.sin(t*8.4 + 0.6)*0.038; }
function dustLaneWidth(t) { return 0.032 + (0.5 + 0.5*Math.sin(t*5.8 + 2.1))*0.024; }
function addStream(count, points, secondary = false) {
  ...
  while (created < count && guard < count * 12) {
    ...
    const z = Math.max(-0.88, Math.min(0.92,
      gaussian()*(secondary ? 0.26 : 0.34) + (random()-0.5)*0.15));
    const densityCenter = Math.exp(-Math.pow((t - 0.52)/0.34, 2));
    const size = 0.58 + random()*(1.12 + densityCenter*0.42);
    const alpha = (0.078 + random()*0.28) * (0.80 + densityCenter*0.30) * (secondary ? 0.80 : 1);
    pushStar(x, y, z, pickColor(secondary ? 0.88 : 0.98), size, alpha,
      random()*57, 0.55 + random()*1.25, 1);
    created++;
  }
}
function addMidStars(count) {
  for (let i = 0; i < count; i++) {
    ...
    pushStar(frame.x + frame.nx*offset, frame.y + frame.ny*offset, -0.35 + random()*1.10,
      pickColor(1.05), 1.35 + Math.pow(random(), 2.1)*1.75,
      0.34 + random()*0.48, random()*73, 0.58 + random()*1.15, 1);
  }
}
function addBrightStars(count) {
  for (let i = 0; i < count; i++) {
    ...
    const offset = gaussian()*streamWidth(t, false)*0.78;
    pushStar(frame.x + frame.nx*offset, frame.y + frame.ny*offset, -0.12 + random(),
      pickColor(1.16), 3.0 + Math.pow(random(), 1.45)*4.0,
      0.78 + random()*0.20, random()*91, 0.48 + random()*0.95, 2);
  }
}
...
function addHeroStars() {
  HERO_STARS.forEach((star, index) => {
    ...
    pushStar(frame.x + frame.nx*star.offset, frame.y + frame.ny*star.offset,
      0.58 + (index % 3)*0.09, star.color, star.size, 1.0,
      17 + index*8.31, 0.42 + (index % 4)*0.11, 3);
  });
}
...
function bindAttribute(name, size, offsetFloats) {
  ...
  gl.vertexAttribPointer(location, size, gl.FLOAT, false,
    STRIDE*FLOAT_BYTES, offsetFloats*FLOAT_BYTES);
}
...
const compositeUniforms = {
  ...
  blooms: [0,1,2].map(i => gl.getUniformLocation(compositeProgram, `uBloom${i}`)),
...
const state = {
  ...
  startTime: performance.now(), lastFrame: 0,
};
function rebuildTargets() {
  destroyTarget(sceneTarget);
  destroyTarget(bloomSourceTarget);
  for (const level of bloomLevels) { destroyTarget(level.a); destroyTarget(level.b); }
  ...
  for (const scale of [0.5, 0.25, 0.125]) {
    ...
    bloomLevels.push({ a: createTarget(width, height), b: createTarget(width, height) });
  }
}
function resize() {
  ...
  state.dpr = dpr; state.width = width; state.height = height;
  state.aspect = window.innerWidth / Math.max(window.innerHeight, 1);
  if (!changed) return;
  canvas.width = width; canvas.height = height; rebuildTargets();
}
...
function bloomPass(source, target, direction, extract) {
  ...
  gl.uniform1f(bloomUniforms.threshold, useHDR ? 0.16 : 0.12);
...
function composite() {
  ...
  gl.uniform1f(compositeUniforms.bloomIntensity, 0.52);
  gl.drawArrays(gl.TRIANGLES, 0, 3);
}
...
function drawStarPass(target, bloomOnly, seconds, exposure) {
  gl.bindFramebuffer(gl.FRAMEBUFFER, target.framebuffer);
  gl.viewport(0, 0, state.width, state.height);
  gl.clearColor(bloomOnly ? 0 : 0.0015, bloomOnly ? 0 : 0.0030, bloomOnly ? 0 : 0.0060, bloomOnly ? 0 : 1);
...
function render(now) {
  ...
  const dt = Math.min(0.05, Math.max(0.001,
    state.lastFrame ? (now-state.lastFrame)/1000 : 0.016));
  state.lastFrame = now;
  if (reducedMotion) {
    state.pointer[0] = 0; state.pointer[1] = 0; state.pointerActive = 0;
  } else {
