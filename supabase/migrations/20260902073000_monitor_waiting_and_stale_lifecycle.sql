...
alter table public.listing_task_audits
  ...
  check (status = any (array['running'::text, 'completed'::text, 'failed'::text, 'cancelled'::text, 'review'::text, 'ready'::text, 'waiting'::text]));

create or replace function public.normalize_listing_task_audit_waiting_status()
returns trigger
language plpgsql
set search_path = ''
as $function$
begin
  if lower(coalesce(new.status, '')) = 'running'
     and upper(coalesce(new.result_data ->> 'job_status', '')) = 'WAITING_SOURCE_INTERACTION' then
    new.status := 'waiting';
...
create trigger listing_task_audits_normalize_waiting
before insert or update on public.listing_task_audits
for each row
execute function public.normalize_listing_task_audit_waiting_status();
...
where lower(coalesce(status, '')) = 'running'
  ...
  and upper(coalesce(result_data ->> 'job_status', '')) = 'WAITING_SOURCE_INTERACTION';
...
begin
  ...
  select jsonb_build_object(
    ...
    'hours', v_hours,
    'days', v_days,
    'classification', 'whole_product_lifecycle_v2',
